...
- **配置与依赖注入**：`Infrastructure/AzureOpenAiConfig.cs` 从环境变量读取 `AZURE_OPENAI_ENDPOINT`、`AZURE_OPENAI_API_KEY`、`AZURE_API_VERSION`、`ANTHROPIC_AUTH_TOKEN`，并标准化 Azure OpenAI 端点（处理 api-version）。
- **请求转换与代理**：
